...
import { JobCard } from '@/app/dashboard/(booster)/boosts/_components/JobCard'
import type { BoostDisplay } from '@/app/dashboard/(booster)/boosts/_components/JobCard'
    ...
    <div className="flex h-full items-center justify-center px-6">
      <div className="w-full max-w-md rounded-xl border border-yellow-500/20 bg-yellow-500/5 p-8 text-center">
        ...
        <Button asChild className="w-full gap-2">
          <Link href={`/admin/boosts/${orderId}?chat=1`}>
            Go to Active Boost
...
export default async function AdminBoostsPage() {
  ...
  if (!profile || !['admin', 'booster'].includes(profile.role)) redirect('/admin')
