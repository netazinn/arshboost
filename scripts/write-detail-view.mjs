...
const TIER_COLORS: Record<string, string> = {
  Iron: 'bg-[#8b8b8b]', Bronze: 'bg-[#cd7f32]', Silver: 'bg-[#c0c0c0]',
  Gold: 'bg-[#ffd700]', Platinum: 'bg-[#00e5c0]', Emerald: 'bg-[#50c878]',
  Diamond: 'bg-[#7b9cff]', Master: 'bg-[#c855f5]', Masters: 'bg-[#c855f5]',
  GrandMaster: 'bg-[#f55c55]', Challenger: 'bg-[#f0c040]',
  Immortal: 'bg-[#f55c72]', Ascendant: 'bg-[#4ec27a]', Radiant: 'bg-[#f0c040]',
}

function TierDot({ tier }: { tier: string }) {
  const color = TIER_COLORS[tier] ?? 'bg-[#4a4a4a]'
            ...
            <div className="flex flex-col gap-5">
              <SectionCard title="Boost Data">
                <div className="grid grid-cols-3 gap-x-6 gap-y-6">
                  <DataField label="Start Tier"><TierDot tier={d.startTier} />{d.startTier}</DataField>
                  <DataField label="Start Division">{d.startDivision || '—'}</DataField>
                  <DataField label="End Tier"><TierDot tier={d.endTier} />{d.endTier}</DataField>
                  {d.startRR && (
...
console.log('✓ OrderDetailView.tsx written successfully', out)
