...
function ChatView({ order, d, gameIcon, onBack }: ChatViewProps) {
  ...
  return (
    <>
      <div className="flex min-h-0 flex-1 gap-5">
              ...
              {messages.map((msg) => {
                ...
                return (
                  <div key={msg.id} className={\`flex \${isClient ? 'justify-end' : 'justify-start'}\`}>
                    <div className={\`max-w-[70%] \${isClient ? 'order-last' : ''}\`}>
                      {!isClient && (
        ...
        {/* ── Shared info sidebar (30%) ─────────────────────────────────── */}
        <div className="flex min-h-0 flex-[30] flex-col gap-4 overflow-y-auto">
          ...
          {/* Rank info */}
          {(d.startTier !== '—' || d.endTier !== '—') && (
            <div className="rounded-md border border-[#2a2a2a] bg-[#191919] px-5 py-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="flex flex-col gap-2">
                  <span className="font-mono text-[10px] tracking-[-0.07em] text-[#6e6d6f] uppercase">Current Rank</span>
                  <div className="flex items-center gap-2">
                    <TierIcon tier={d.startTier} />
                    <span className="font-mono text-xs tracking-[-0.07em] text-white">{d.startTier}{d.startDivision ? ' ' + d.startDivision : ''}</span>
                  </div>
                </div>
                <div className="flex flex-col gap-2">
                  <span className="font-mono text-[10px] tracking-[-0.07em] text-[#6e6d6f] uppercase">Target Rank</span>
                  <div className="flex items-center gap-2">
                    <TierIcon tier={d.endTier} />
                    <span className="font-mono text-xs tracking-[-0.07em] text-white">{d.endTier}</span>
                  </div>
                </div>
              </div>
            </div>
...
export function OrderDetailView({ order, defaultChat = false }: { order: Order; defaultChat?: boolean }) {
  const router     = useRouter()
  const searchParams = useSearchParams()
  const [loginsModalOpen, setLoginsModalOpen] = useState(false)
  const [chatOpen, setChatOpen] = useState(defaultChat || searchParams.get('chat') === '1')
    ...
    <>
      <div className="flex-1 min-h-0 overflow-hidden w-full">
        <div className="w-full max-w-[1440px] mx-auto px-8 py-10 flex flex-col gap-7 h-full">
          ...
          {/* Content */}
          {chatOpen ? (
            <ChatView order={order} d={d} gameIcon={gameIcon} onBack={() => setChatOpen(false)} />
          ) : (
            /* 2-column layout */
...
console.log('✓ OrderDetailView.tsx written')
